...
public class llrbTree <Key extends Comparable<Key>, Value> {
	...
	private Node splitFourNode(Node node) {
		...
		assert(isRed(node)): "Node was not red during split four node";
		flipColors(node);
	...
	private void flipColors(Node node) {
		assert !isRed(node):"Failed. Parent node is not Black";
		assert !isRed(node.right):"Failed. node.right is not Red";
		assert !isRed(node.left):"Failed. node.left is not Red";
	...
	public static void main(String[] args) {
		llrbTree<Integer, String> llrbt = new llrbTree();
